...
class BINDTI(nn.Module):
    def __init__(self, device='cuda:7', **config):
        ...
        self.drug_extractor = LncRNACNN(protein_emb_dim, num_filters, kernel_size, protein_padding)
        ...
        # self.protein_extractor = LncRNACNN(protein_emb_dim, num_filters, kernel_size, protein_padding)
    ...
    def forward(self, bg_d, v_p, mode="train"):
        ...
        f, v_d_I, v_p_I, att = self.cross_intention(drug=v_d, protein=v_p)#f:[64, 256]

        #ablation
        # v_d_pool = torch.max(v_d, dim=1)[0]  # shape: (B, D)
        # v_p_pool = torch.max(v_p, dim=1)[0]  # shape: (B, D)
        # f = torch.cat((v_d_pool, v_p_I), dim=1)

        score = self.mlp_classifier(f)
        if mode == "train":
            return v_d_I, v_p_I, f, score
        elif mode == "eval":
            return v_d_I, v_p_I, score, att
            # return v_d, v_p, score, None

class LncRNACNN(nn.Module):
    def __init__(self, embedding_dim, num_filters, kernel_size, padding=True):
        super(LncRNACNN, self).__init__()
        if padding:
